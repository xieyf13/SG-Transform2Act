...
class DevHumanoidHN(Agent):
    ...
    def __init__(self, agent_id, cfg, xml_path=None, n_agents=2, **kwargs):
        ...
        super(DevHumanoidHN, self).__init__(agent_id, xml_path, n_agents, **kwargs)
    ...
    def _get_obs(self, stage=None):
        ...
        for body in agent_body.iter('body'):
            ...
            if cur_name == "torso":
                obs_i = [mat.T @ (self.env.data.body(self.scope + "/" +cur_name).xipos - root_pos), mat.T @ other_pos, np.array([0,0,9.8]), mat.T @ qvel[0:3], mat.T @ qvel[3:6], self.env.data.body(self.scope + "/" +cur_name).xipos[2:3],np.zeros(6)]
            else:
                ...
                if qe - qs >= 1:
                    ...
                    angle = np.zeros(6)
                obs_i = [mat.T @ (self.env.data.body(self.scope + "/" +cur_name).xipos - root_pos), mat.T @ other_pos,  np.array([0,0,9.8]),  np.zeros(6), self.env.data.body(self.scope + "/" +cur_name).xipos[2:3],angle]
