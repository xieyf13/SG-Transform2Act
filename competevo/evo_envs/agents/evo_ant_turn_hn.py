...
def normalize_vector(v, dim, eps=1e-6):
    # 计算范数并添加 eps 以避免除零错误
    norm = np.linalg.norm(v, ord=2, axis=dim, keepdims=True)
    return v / (norm + eps)
...
def construct_3d_basis_from_1_vectors(v1):
    ...
    # 将 u1, u2, u3 堆叠成矩阵
    mat = np.stack([u1, u2, u3], axis=-1)  # (N, L, 3, 3)
    return mat


class EvoAntTurnHN(Ant):

    def __init__(self, agent_id, cfg, xml_path=None, n_agents=2):
        super(EvoAntTurnHN, self).__init__(agent_id, xml_path, n_agents)
        self.cfg = cfg
    ...
    def get_sim_obs(self):
        ...
        torso_quat = self.env.data.qpos[3:7]
        self.torso_euler = self.quat2euler(torso_quat)
        heading = np.array([np.cos(self.torso_euler[2]), np.sin(self.torso_euler[2]),0])
        mat = construct_3d_basis_from_1_vectors(heading)
        ...
        for i, body in enumerate(self.robot.bodies):
            ...
            if i == 0:
                ...
                obs_i = [mat.T @ (self.env.data.body(self.scope + "/" +body.name).xipos - root_pos), mat.T @ other_pos, np.array([0,0,9.8]), quat , mat.T @ qvel[:3], mat.T @ qvel[3:6], self.env.data.body(self.scope + "/" +body.name).xipos[2:3],np.zeros(2)]
            else:
                ...
                obs_i = [mat.T @ (self.env.data.body(self.scope + "/" +body.name).xipos - root_pos), mat.T @ other_pos,  np.array([0,0,9.8]), quat, np.zeros(6), self.env.data.body(self.scope + "/" +body.name).xipos[2:3],angle]
            # if 'root_offset' in self.sim_specs:
